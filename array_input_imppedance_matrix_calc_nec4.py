...
Software_name = 'Array Input Impedance Matrix Calculator NEC4'
...
IFedDipImag = np.full((1, num_of_freq, 5, max_loads_num), 0.0)  # (Number of files analyzed, freq step, dipole, LoadNum)

# *** Creating folder for results ***
result_path = 'Results_'+file_name
...
for FileNum in range (TotFileNum):
    ...
    for step in range(num_of_frequencies):
        ...
        print('  ------------------------------ \n')
        print('  Self full impedance of the dipole = ', Rinp[FileNum, step, 1+(Active-1)*2, 0],
              Xinp[FileNum-1, step, 1+(Active-1)*2, 0], ' Ohm')
        print('  Current in fed point of active dipole = ', IFedDipReal[FileNum, step, 1+(Active-1)*2, 0],
              IFedDipImag[FileNum-1, step, 1+(Active-1)*2, 0], ' Amp. \n\n')
        ...
        for j in range(LoadNum):  # loop by number of loads
            ...
            if segments_with_loads[j] < 1:  # if the number of segment = 1 we need to return to the beginning of the line
                print('\n\n   Error! No segments with loads')
                sys.exit()
            if segments_with_loads[j] > 1:  # if the number of segment > 2 we need to skip lines
                for i in range(1, segments_with_loads[j] - 2):
                    line = Data_File.readline()   # Skip several lines
            ...
            if print_or_not == 1:
                print('  Current in the load No ', j+1, ' = ', IFedDipReal[FileNum, step, 2+(Active-1)*2, j],
                      IFedDipImag[FileNum, step, 2+(Active-1)*2, j], ' Amp')
    ...
    for step in range(num_of_frequencies):  # Loop by frequencies for calculations

        print('  For frequency = ', frequency_list[step], ' MHz')
        print('  ------------------------------')
...
MutFullImpReFile.write('  Frequency,MHz  ' + ''.join('    WireNo='+str(wires_with_loads[i]).zfill(5) +
                                                     '   ' for i in range(LoadNum)) + '\n')
for LineNum in range(num_of_frequencies):
    data = ['   %14.6e  ' % np.real(Zinp[FileNum, LineNum, 2, i]) for i in range(LoadNum)]
    MutFullImpReFile.write(('    %8.4f    ' % frequency_list[LineNum]) + ''.join(data) + '   \n')


# Mutual full impedances (imag part).txt
MutFullImpImFile.write('  Frequency,MHz  ' + ''.join('    WireNo='+str(wires_with_loads[i]).zfill(5) +
                                                     '   ' for i in range(LoadNum)) + '\n')
for LineNum in range(num_of_frequencies):
    data = ['   %14.6e  ' % np.imag(Zinp[FileNum, LineNum, 2, i]) for i in range(LoadNum)]
    MutFullImpImFile.write(('    %8.4f    ' % frequency_list[LineNum]) + ''.join(data) + '   \n')


# Mutual full impedances (Module).txt
MutFullImpModFile.write('  Frequency,MHz  ' + ''.join('    WireNo='+str(wires_with_loads[i]).zfill(5) +
                                                      '   ' for i in range(LoadNum)) + '\n')
for LineNum in range(num_of_frequencies):
    data = ['   %14.6e  ' % np.absolute(Zinp[FileNum, LineNum, 2, i]) for i in range(LoadNum)]
    MutFullImpModFile.write(('    %8.4f    ' % frequency_list[LineNum]) + ''.join(data) + '   \n')
...
for LineNum in range(num_of_frequencies):
    ...
    SelfFullImpReFile.write(('    %8.4f    ' % frequency_list[LineNum]) + ''.join(data) + ' \n')
...
for LineNum in range(num_of_frequencies):
    ...
    SelfFullImpImFile.write(('    %8.4f    ' % frequency_list[LineNum]) + ''.join(data) + ' \n')
...
for LineNum in range(num_of_frequencies):
    ...
    SelfFullImpModFile.write(('    %8.4f    ' % frequency_list[LineNum]) + ''.join(data) + ' \n')

# Frequency dependence of dipole input impedance
fig = plt.figure(figsize=(10, 6))
rc('font', weight='normal')
plt.plot(frequency_list[1:len(frequency_list)], np.real(Zinp[FileNum, 0:len(frequency_list)-1, 1, 0]),
         color='r', linestyle='-', linewidth='1.00', label='Re(Zinp)')
plt.plot(frequency_list[1:len(frequency_list)], np.imag(Zinp[FileNum, 0:len(frequency_list)-1, 1, 0]),
         color='b', linestyle='-', linewidth='1.00', label='Im(Zinp)')
plt.plot(frequency_list[1:len(frequency_list)], np.absolute(Zinp[FileNum, 0:len(frequency_list)-1, 1, 0]),
         color='violet', linestyle='-', linewidth='1.00', label='Abs(Zinp)')
...
rc('font', weight='normal')
for step in range(num_of_frequencies):
    plt.plot(loads, np.absolute(Zinp[FileNum, step, 2, 0:LoadNum]),
             linestyle='-', linewidth='1.00', label=str(frequency_list[step]) + ' MHz')
plt.xlabel('Number of load')
plt.ylabel('Abs (Z), Ohm')
# plt.suptitle(SupTitle, fontsize=10, fontweight='bold')
plt.title('NEC modeling results. Dependence of mutual input impedance on dipole number for all frequencies',
          fontsize=7, x=0.46, y=1.005)
plt.grid(b=True, which='both', color='silver', linestyle='-')
plt.legend(loc='center left', fontsize=6, bbox_to_anchor=(1, 0.5))
pylab.savefig(result_path + "/Mutual impedances (modules).png", bbox_inches='tight', dpi=160)
# plt.show()
plt.close('all')
