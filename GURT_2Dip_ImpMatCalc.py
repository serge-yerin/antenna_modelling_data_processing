...
def IntegrTrap2x(a, b, c, d, m, n, Points):
    ...
    IntegrResult = ((((b) - (a)) * pi / 180.0) / m) * ((((d) - (c)) * pi / 180.0) / n) * (
                (Points[a, c] + Points[b, d]) + Sum)
    return IntegrResult
...
ETHcmplx = np.full((10, ArrayInputNum, 181, 361),
                   0.0 + 1j * 0.0)  # [FreqStep, DipNo, theta, phi] Theta components of E-field
EPHcmplx = np.full((10, ArrayInputNum, 181, 361),
                   0.0 + 1j * 0.0)  # [FreqStep, DipNo, theta, phi] Phi components of E-field
...
for FileNum in range(NECfileNum):  # Main loop by NEC output files
    ...
    for i in range(LoadNum):
        print('  Wire No ', wires_with_loads[i], '  Segm No', segments_with_loads[i], '  Resistance = ',
              loads_values[i])
    ...
    for step in range(NoOfFrequencies):
        ...
        print('  ---------------------------- \n')
        print('  Self full impedance of the dipole = ', Rinp[step, FileNum, FileNum], Xinp[step, FileNum, FileNum],
              ' Ohm')
        print('  Current in fed point of active dipole = ', IFedDipReal[step, FileNum, FileNum],
              IFedDipImag[step, FileNum, FileNum], ' Amp. \n')
        ...
        for j in range(LoadNum):  # loop by number of loads
            ...
            if (segments_with_loads[
                j] < 1):  # if the number of segment = 1 we need to return to the begining of the line
                stop
            ...
            print('  Current in the load No ', j + 1, ' = ', IFedDipReal[step, FileNum, k],
                  IFedDipImag[step, FileNum, k], ' Amp')
        ...
        for k in range(65341):
            ...
            phiInt = int(phi1)
            ETHcmplx[step, FileNum, thetaInt, phiInt] = np.complex((Etheta * np.cos(ang1 * pi / 180)),
                                                                   (Etheta * np.sin((ang1 * pi / 180))))
            EPHcmplx[step, FileNum, thetaInt, phiInt] = np.complex((Ephi * np.cos(ang2 * pi / 180)),
                                                                   (Ephi * np.sin((ang2 * pi / 180))))
...
for step in range(NoOfFrequencies):  # Loop by frequencies for results files writing
    ...
    for k in range(ArrayInputNum):
        data = ['   {:+14.8e}   {:+14.8e}'.format(np.real(IFedDip[step, k, i]), np.imag(IFedDip[step, k, i])) for i in
                range(ArrayInputNum)]
    ...
    for k in range(ArrayInputNum):
        data = ['   {:+14.8e}   {:+14.8e}'.format(np.real(Zinp[step, k, i]), np.imag(Zinp[step, k, i])) for i in
                range(ArrayInputNum)]
    ...
    for k in range(ArrayInputNum):
        data = ['   {:+14.8e}   {:+14.8e}'.format(np.real(RSigm[step, k, i]), np.imag(RSigm[step, k, i])) for i in
                range(ArrayInputNum)]
    ...
    for k in range(ArrayInputNum):
        for t in range(91):
            data = [' {:+14.8e}'.format(np.real(ETHcmplx[step, k, t + 90, p] / IFedDip[step, k, k])) for p in
                    range(361)]
            CurrentFile.write('' + ''.join(data) + ' \n')
    for k in range(ArrayInputNum):
        for t in range(91):
            data = [' {:+14.8e}'.format(np.imag(ETHcmplx[step, k, t + 90, p] / IFedDip[step, k, k])) for p in
                    range(361)]
            CurrentFile.write('' + ''.join(data) + ' \n')
    CurrentFile.close()
    ...
    for k in range(ArrayInputNum):
        for t in range(91):
            data = [' {:+14.8e}'.format(np.real(EPHcmplx[step, k, t + 90, p] / IFedDip[step, k, k])) for p in
                    range(361)]
            CurrentFile.write('' + ''.join(data) + ' \n')
    for k in range(ArrayInputNum):
        for t in range(91):
            data = [' {:+14.8e}'.format(np.imag(EPHcmplx[step, k, t + 90, p] / IFedDip[step, k, k])) for p in
                    range(361)]
            CurrentFile.write('' + ''.join(data) + ' \n')
    CurrentFile.close()
